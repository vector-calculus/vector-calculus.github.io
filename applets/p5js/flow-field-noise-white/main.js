...
function drawBackground() {
  ctx.fillStyle = "rgb(249, 250, 251)"; // Light grey background
...
function drawFlowField() {
  ctx.strokeStyle = "grey"; // Set stroke color to grey
  ctx.lineWidth = 1.8; // Set line thickness
  ...
  for (let x = 0; x < columns; x++) {
    for (let y = 0; y < rows; y++) {
      ...
      let x1 = x * size;
      let y1 = y * size;
      let x2 = x1 + field[x][y].x * size * 2;
      let y2 = y1 + field[x][y].y * size * 2;
      ...
      // Draw the arrowhead
      let arrowSize = 5; // Size of arrowhead
      let angle = Math.atan2(y2 - y1, x2 - x1); // Calculate the angle of the vector
